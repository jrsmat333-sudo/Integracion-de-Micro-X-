...
CREATE TABLE attraction (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), location_id UUID REFERENCES locations(id), slug VARCHAR(200) NOT NULL UNIQUE, name VARCHAR(150) NOT NULL, description_short VARCHAR(255), description_full TEXT, image_url TEXT, address TEXT, latitude NUMERIC(9,6), longitude NUMERIC(9,6), meeting_point TEXT, rating_average NUMERIC(3,2) DEFAULT 0.00, rating_count INTEGER DEFAULT 0, min_age SMALLINT, max_group_size SMALLINT, difficulty_level VARCHAR(20), is_active BOOLEAN DEFAULT TRUE, is_published BOOLEAN DEFAULT FALSE, managed_by_id UUID, deleted_at TIMESTAMPTZ, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ);
